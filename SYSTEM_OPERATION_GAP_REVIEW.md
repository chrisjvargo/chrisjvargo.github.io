...
| OPS005 | dv_publication_evidence | open_gap | hypothesis_status_counts={'unresolved_required_data_unavailable': 14}; gap_register_rows=14; mapped_unresolved_gaps=14/14; gap_register_md_exists=True | Acquire/verify required case-level data and model artifacts using `DV_EVIDENCE_GAP_REGISTER.md`, then regenerate the DV public release. |
...
| OPS007 | deployment_state | open_gap | branch=main; working_tree_changes=M Makefile \|  M SYSTEM_OPERATION_GAP_REVIEW.md \|  M src/system_operation_audit.py \|  M tests/dv/test_system_operation_audit.py \| ?? DV_EVIDENCE_GAP_REGISTER.md \| ?? dv_publication/evidence_gap_register.csv \| ?? src/dv_evidence_gap_register.py | Commit local changes, push, and let the GitHub Pages workflow deploy. |
